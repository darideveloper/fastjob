...
#### Scenario: Casing variants are de-duplicated
- **GIVEN** two `Company` rows with `area = "Tecnología"` and `area = "tecnología"`
- **WHEN** the endpoint is called
- **THEN** `areas` contains exactly one entry whose canonical casing matches the most-frequent variant in the database
