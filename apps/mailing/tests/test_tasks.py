...
def test_task_respects_user_area_filter(
    ...
    from apps.companies.models import Area
    a_fin, _ = Area.objects.get_or_create(name="Finanzas")
    a_tec, _ = Area.objects.get_or_create(name="Tecnología")
    a_tec_ind, _ = Area.objects.get_or_create(name="Tecnología Industrial")

    Company.objects.create(email="a@x.com", name="A", area=a_fin, location=None)
    ...
    assert chosen_company.area.name.lower() == "tecnología"
